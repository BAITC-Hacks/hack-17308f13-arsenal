...
export function validateScenario(
  ...
  if (Array.isArray(input) && input.length > 5) {
    issues.push({
      code: "count",
      message: "Выбраны все 5 мероприятий. Удалите одно, чтобы заменить его.",
    });
    return finish(0);
  }
  ...
  if (new Set(decisions.map((d) => d.measureId)).size !== decisions.length)
    issues.push({
      code: "duplicate",
      message: "Это мероприятие уже в плане. Меры не должны повторяться.",
    });
  let spent = 0;
  ...
  if (spent > 100)
    issues.push({
      code: "budget",
      message: `План стоит ${spent} ед. Бюджет — 100. Уберите другое мероприятие.`,
    });
  for (const [dir, count] of Object.entries(counts))
    if (count > 2)
      issues.push({
        code: "direction",
        message: `«${directions[dir as Direction]}»: уже выбраны 2 меры — это максимум. Уберите одну, чтобы заменить её.`,
      });
  for (const [a, b, same] of [
    ...
    if (x && y && (!same || x.districtId === y.districtId)) {
      ...
      issues.push({
        code: "conflict",
        message: `${names.join(" и ")} несовместимы${same ? " в одном районе. Выберите другой район" : ". Оставьте в плане только одну из этих мер"}.`,
      });
    }
  }
  return finish(spent);
...
export function previewDecision(
  ...
  return {
    ...
    errors: validation.issues.map((i) =>
      i.code === "budget"
        ? `Нужно ${cost} ед., осталось ${remaining}. Уберите другое мероприятие.`
        : i.message,
    ),
  };
}
